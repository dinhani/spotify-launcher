...
- Picking and playing must be fast: type to search, arrows to navigate, `Enter` opens the artist in Spotify, `Shift+Enter` opens it in Last.fm, `Esc` clears the search from anywhere and returns to it from a card, `Alt+1` and `Alt+2` cycle sort and group (`Shift` goes back). Mobile must stay comfortable to tap.
