...
- Picking and playing must be fast: type to search, arrows to navigate, `Enter` opens the artist in Spotify, `Shift+Enter` opens it in Last.fm, `Esc` clears the search from anywhere and returns to it from a card, `Ctrl+1`, `Ctrl+2` and `Ctrl+3` cycle filter, sort and group (`Shift` goes back; filter cycling also focuses the sidebar), `Ctrl+4` focuses the card grid, `Ctrl+Up/Down` cycle the filter and `Ctrl+Left/Right` the sort (not inside the search input, where they move by word). Mobile must stay comfortable to tap.
